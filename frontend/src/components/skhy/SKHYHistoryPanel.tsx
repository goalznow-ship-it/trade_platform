...
export function SKHYHistoryPanel({ history }: Props) {
  ...
  if (!signals.length && perf.status === "no_trades_yet") {
    return (
      <div className="p-3 border-b border-gray-800/60">
        <div className="text-[11px] text-gray-400 font-semibold uppercase tracking-wider mb-2">History</div>
  ...
  return (
    <div className="p-3 border-b border-gray-800/60">
      <div className="text-[11px] text-gray-400 font-semibold uppercase tracking-wider mb-2">Performance</div>

      {perf.win_rate != null && (
        <div className="grid grid-cols-3 gap-1 mb-2">
          <MetricBox label="Win Rate" value={strVal(perf.win_rate)} color="text-green-400" />
          <MetricBox label="Profit Factor" value={perf.profit_factor === Infinity ? "∞" : strVal(perf.profit_factor)} color="text-blue-400" />
          <MetricBox label="Total" value={strVal(perf.total_signals)} color="text-gray-300" />
          <MetricBox label="LONG WR" value={perf.long_win_rate != null ? `${String(perf.long_win_rate)}%` : "N/A"} color="text-green-400" />
          <MetricBox label="SHORT WR" value={perf.short_win_rate != null ? `${String(perf.short_win_rate)}%` : "N/A"} color="text-red-400" />
          <MetricBox label="Avg R:R" value={perf.average_rr != null ? strVal(perf.average_rr) : "N/A"} color="text-yellow-400" />
        </div>
      )}

...
export function SKHYTriggerPanel({ triggers, scores }: Props) {
  ...
  return (
    <div className="border-b border-gray-800/60 p-3">
      <div className="text-[11px] text-gray-400 font-semibold uppercase tracking-wider mb-2">Triggers</div>
      ...
      <div className={cn("rounded border p-2 mb-2", longActive ? "border-green-500/40 bg-green-500/5" : "border-gray-700/40 bg-gray-800/20")}>
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-1">
            ...
            <span className="text-[11px] font-semibold text-gray-300">LONG</span>
          </div>
        ...
        {triggers.bullish_invalidation != null && (
          <div className="flex items-center gap-1 mt-1 text-[10px] text-red-400/70">
            ...
            <span>Invalidation: ${numVal(triggers.bullish_invalidation).toFixed(2)}</span>
          </div>
        )}
      </div>
      ...
      <div className={cn("rounded border p-2 mb-2", shortActive ? "border-red-500/40 bg-red-500/5" : "border-gray-700/40 bg-gray-800/20")}>
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-1">
            ...
            <span className="text-[11px] font-semibold text-gray-300">SHORT</span>
          </div>
        ...
        {triggers.bearish_invalidation != null && (
          <div className="flex items-center gap-1 mt-1 text-[10px] text-red-400/70">
            <XCircle className="w-2.5 h-2.5" />
            <span>Invalidation: ${numVal(triggers.bearish_invalidation).toFixed(2)}</span>
          </div>
        )}
      </div>

      {/* Trade Plan (only if confidence >= 70) */}
      {showTradePlan && longActive && (
        <div className="rounded border border-green-500/30 bg-green-500/5 p-2">
          <div className="text-[11px] font-semibold text-green-400 mb-1">⚡ LONG Trade Plan</div>
          <div className="text-[10px] text-gray-400 space-y-0.5">
            <div className="flex justify-between"><span>Entry Zone</span><span className="font-mono text-gray-300">${numVal(triggers.long_trigger_price).toFixed(2)}</span></div>
            <div className="flex justify-between"><span>Stop Loss</span><span className="font-mono text-red-400">${numVal(triggers.bullish_invalidation).toFixed(2)}</span></div>
            <div className="flex justify-between"><span>Risk/Reward</span><span className="font-mono text-green-400">~1:2</span></div>
          </div>
        </div>
      )}
      ...
      {showTradePlan && shortActive && (
        <div className="rounded border border-red-500/30 bg-red-500/5 p-2">
          <div className="text-[11px] font-semibold text-red-400 mb-1">⚡ SHORT Trade Plan</div>
          <div className="text-[10px] text-gray-400 space-y-0.5">
            <div className="flex justify-between"><span>Entry Zone</span><span className="font-mono text-gray-300">${numVal(triggers.short_trigger_price).toFixed(2)}</span></div>
            <div className="flex justify-between"><span>Stop Loss</span><span className="font-mono text-red-400">${numVal(triggers.bearish_invalidation).toFixed(2)}</span></div>
            <div className="flex justify-between"><span>Risk/Reward</span><span className="font-mono text-green-400">~1:2</span></div>
          </div>
        </div>
      )}
      ...
      {!showTradePlan && (
        <div className="flex items-center gap-1.5 px-2 py-1.5 rounded bg-yellow-500/5 border border-yellow-500/20">
          ...
          <div>
            ...
            <p className="text-[9px] text-gray-500">Confidence {confidence}% - təsdiq gözlənilir</p>
          </div>
        </div>
      )}
    </div>
